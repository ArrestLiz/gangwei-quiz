...
import { Lock, Eye, EyeOff, Loader2, ChevronDown, User } from 'lucide-react'
...
export default function PasswordGate() {
  ...
  const isAdmin = selectedId && accounts.find(a => a.id === selectedId)?.role === 'admin'

  async function handleSubmit(e) {
    e.preventDefault()
    if (!selectedId) { setError('请先选择账号'); return }
    if (!password) { setError('请输入密码'); return }
    if (!isAdmin && !name.trim()) { setError('请输入姓名'); return }
    const result = await login(selectedId, password, name.trim())
    if (!result.ok) {
          ...
          {!isAdmin && selectedId && (
            <div className="relative">
              ...
              <input
                ...
                placeholder="请输入姓名"
                disabled={unlocking}
                className={`w-full pl-9 pr-3 py-2 border rounded-lg outline-none transition-colors text-sm disabled:opacity-50
                  ${error && !name.trim() ? 'border-red-400' : 'border-gray-200 focus:border-brand-light'}`}
              />
            </div>
          )}
